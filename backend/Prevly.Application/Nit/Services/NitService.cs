...
using NitEntity = Prevly.Domain.Entities.Nit;

namespace Prevly.Application.Nit.Services;

public sealed class NitService(
    INitRepository nitRepository,
...
) : INitService
{
    ...
    public Task<PagedResult<NitEntity>> GetPaginatedAsync(FilterNitDto parameters)
    {
        var filters = new List<FilterDefinition<NitEntity>>();

        if (!string.IsNullOrWhiteSpace(parameters.Number))
        {
            var numberPattern = Regex.Escape(parameters.Number.Trim());
            filters.Add(Builders<NitEntity>.Filter.Regex(
                nit => nit.Number,
        ...
        {
            filters.Add(Builders<NitEntity>.Filter.Eq(
                nit => nit.Status,
        ...
        var filter = filters.Count > 0
            ? Builders<NitEntity>.Filter.And(filters)
            : Builders<NitEntity>.Filter.Empty;

        return nitRepository.GetPaginatedAsync(filter, parameters);
    }

    public async Task<ImportNitsResultDto> ImportFromPdfAsync(
        Stream pdfStream,
        string fileName,
        string contentType,
        string? personId
    ...
    public async Task<ImportNitsResultDto> ImportFromNumbersAsync(
    ...
    )
    {
        ...
        var text = await ExtractTextAsync(pdfStream);
        var nitNumber = ExtractCandidates(text).FirstOrDefault(IsValidNit);
        var dates = ExtractDates(text);

        if (string.IsNullOrWhiteSpace(nitNumber) || dates.Count == 0)
        {
        ...
        var filter = Builders<NitEntity>.Filter.And(
            Builders<NitEntity>.Filter.Eq(x => x.Number, nitNumber),
        ...
        var nitEntity = await nitRepository.GetOneAsync(filter);
        if (nitEntity?.Id is null)
        {
            return new ContributionDetailsImportResultDto(
                ProcessedFiles: 1,
                UpdatedNits: 0,
        ...
        return new ContributionDetailsImportResultDto(
            ...
            NotFoundNits: 0,
            InvalidFiles: 0,
            UpdatedNitNumbers: [nitNumber]
        );
    }

    public async Task BindPersonToNitAsync(BindPersonToNitDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.NitId) || string.IsNullOrWhiteSpace(dto.PersonId))
            throw new ArgumentException("NitId e PersonId sao obrigatorios.");

        var person = await personRepository.GetByIdAsync(dto.PersonId);
        if (person is null)
            throw new ArgumentException("Person nao encontrada.");

        var nit = await nitRepository.GetByIdAsync(dto.NitId);
        if (nit?.Id is null)
    ...
    {
        if (dto.NitIds.Count == 0)
            return [];

        var reportItems = new List<NitReportItemDto>(dto.NitIds.Count);

        foreach (var nitId in dto.NitIds.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct())
        {
            var nit = await nitRepository.GetByIdAsync(nitId);
            if (nit?.Number is null)
                continue;

            Domain.Entities.Person? person = null;
            if (!string.IsNullOrWhiteSpace(nit.PersonId))
                person = await personRepository.GetByIdAsync(nit.PersonId);
            ...
            reportItems.Add(new NitReportItemDto(
                ...
                ContributionYears: nit.ContributionYears
            ));
        }
    ...
    public async Task<IReadOnlyCollection<NitEntity>> GetForExportAsync(
        string? query,
        NitStatus? status,
        IReadOnlyCollection<string>? nitIds
    )
    {
        var filterBuilder = Builders<NitEntity>.Filter;
        var filters = new List<FilterDefinition<NitEntity>>();

        var selectedIds = nitIds?
            .Where(x => !string.IsNullOrWhiteSpace(x))
            .Select(x => x.Trim())
            .Distinct()
            .ToList() ?? [];
        ...
        {
            filters.Add(filterBuilder.In(nit => nit.Id, selectedIds));
        }
        else
        {
            if (!string.IsNullOrWhiteSpace(query))
            {
                ...
                filters.Add(filterBuilder.Or(
                    filterBuilder.Regex(nit => nit.Number, pattern),
                    filterBuilder.Regex(nit => nit.OwnershipOwnerName, pattern)
                ));
            }

            if (status.HasValue)
            {
                filters.Add(filterBuilder.Eq(nit => nit.Status, status.Value));
            }
        }
    ...
    private async Task<IReadOnlyCollection<NitEntity>> GetAllByFilterAsync(
        FilterDefinition<NitEntity> filter
    )
    {
        const int pageSize = 500;
        var pageNumber = 1;
        var all = new List<NitEntity>();

        while (true)
        {
            var page = await nitRepository.GetPaginatedAsync(
                filter,
    ...
    private async Task<ImportNitsResultDto> SaveUniqueValidNitsAsync(
    ...
    )
    {
        ...
        {
            var filter = Builders<NitEntity>.Filter.Eq(x => x.Number, number);
            var existing = await nitRepository.GetOneAsync(filter);
            ...
            await nitRepository.CreateAsync(nit);
            inserted++;
        }
